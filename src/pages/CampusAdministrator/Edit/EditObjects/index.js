...
export default function EditObjects() {
  ...
  useEffect(() => {
    async function loadDataObject() {
      const response = await api.get(`api/objects/show/${id}`);
      setNameObject(response.data.name);
      setPlacesId(response.data.campus);
  ...
  async function handleEditObject(e) {
    ...
    try {
      const response = await api.put(`api/objects/${id}`, {
        name: nameObject,
        fk_place_id: placesId
      });
              ...
              />
                ...
                <>
                  <Description
                    ...
                    tip="Área destinada para alteração dos objetos."
                    color="#0B0A0D"
                  />

                  <div className="form-container-campus">
                    <form onSubmit={e => handleEditObject(e)}>
                      ...
                      <div className="select-group">
                        ...
                        <select onChange={(e) => handleSelectPlaces(e)}>
                          {
                            places.map(place => (
