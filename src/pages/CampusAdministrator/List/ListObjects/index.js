...
export default function ListObjects() {
  ...
  useEffect(() => {
    async function loadPlaces() {
      try {
        const response = await api.get(`api/objects/filter?page=${currentPage}`);
  ...
  useEffect(() => {
    async function filterPlaces() {
      try {
        const response = await api.get(`api/objects/filter?name=${search}`);
        setObjects(response.data.data);
      } catch (error) {
        console.log(error)
      }
  ...
  async function DeleteObjects(id) {
    try {
      ...
      <Content toggle={toggle} bg="#f5f5f8">
        ...
        <div className={toggle ? 'main-disabled' : 'main-active'}>
          ...
          <Description
            ...
            tip="Área destinada para o listagem dos objetos."
            color="#111014"
          />
              ...
              <div className="actions">
                ...
                <div className="options">
                  <div className="btn-op-group">
                    <Link to="/campus/registerObject" className="op-link">
                      <MdAdd
                        size={24}
                        color="#3708bc"
                ...
                <Tbody color="#ffffff">
                  {objects.map(object => (
                    <Tr key={object.id}>
                      ...
                      <Td>
                        <Link to={`/campus/editObject/${object.id}`} className="btn-actions">
                          <BsPencil size={20} color="#3708bc" />
                        </Link>
                      </Td>
                      <Td>
                        <Link className="btn-actions" onClick={() => DeleteObjects(object.id)}>
