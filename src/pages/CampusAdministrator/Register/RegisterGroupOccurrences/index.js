...
export default function RegisterGroupOccurrences() {
  ...
  async function handleRegisterGroup(e) {
    ...
    try {
      const response = await api.post('api/groupoccurrences/', {
        name: nameGroup
      });
        ...
        <div className={toggle ? 'main-disabled' : 'main-active'}>
          {
            showModal ? (
              ...
              />
                ...
                <>
                  <Description
                    description="Registrar Grupo de Ocorrência"
                    tip="Área destinada para o registro de novos grupos de ocorrência."
                    color="#0B0A0D"
                  />

                  <div className="form-container-campus">
                    <form onSubmit={e => handleRegisterGroup(e)}>
