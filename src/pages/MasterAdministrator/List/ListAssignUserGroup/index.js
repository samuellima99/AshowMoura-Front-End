...
export default function ListAssingUserGroup() {
  ...
  useEffect(() => {
    async function loadAdmins() {
      try {
        const response = await api.get(`api/users/searchUsersNullGroup?page=${currentPage}`);
      ...
      <Content toggle={toggle} bg="#0B0A0D">
        ...
        <div className={toggle ? 'main-disabled' : 'main-active'}>
          ...
          <Description
            ...
            tip="Área destinada para o listagem dos administradores de Campus sem grupo de usuário."
          />
                ...
                <Tbody>
                  {admins.map(admin => (
                    <Tr key={admin.id}>
                      ...
                      <Td>
                        <Link to={`/master/assignGroup/${admin.id}`} className="btn-actions-master">
                          <MdLink size={20} color="#2CC464" />
                        </Link>
                      </Td>
                      <Td>
